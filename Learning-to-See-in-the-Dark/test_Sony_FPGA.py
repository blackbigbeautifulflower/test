...
with tf.device('/GPU:0'):  # 使用GPU进行推理
    ...
    for test_id in test_ids:
        ...
        # 读取测试数据
        input_patch = np.load(in_file).astype(np.float32)
        gt_patch = np.load(gt_file).astype(np.float32)
        ...
        # 评价指标
        gt_patch_resized = (gt_patch * 255).astype(np.uint8)
        mse = np.mean((output_resized - gt_patch_resized) ** 2)
        ssim = compare_ssim(output_resized, gt_patch_resized, win_size=7, channel_axis=-1)
    ...
    print(f"Testing ended at: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
